...
- The Mojo language basic trait hierarchy has changed to expand first-class
  ...
  [`Copyable`](/docs/std/builtin/value/Copyable) traits no longer require that a
  type provide a `__del__()` method that may be called by the compiler
  ...
  Relatedly, the `UnknownDestructibility` trait is now no longer required, as
  it is equivalent to the new `AnyType` behavior.
...
- Several traits now have default implementations that use reflection to
  ...
  **[`Equatable`](/docs/std/builtin/comparable/Equatable)** - Default `__eq__()`
  compares all fields:
...
- [`alloc()`](/docs/std/memory/unsafe_pointer/alloc/) now has a
  [`debug_assert()`](/docs/std/builtin/debug_assert/debug_assert) ensuring count
  is non-negative.
...
- The `Iterator` trait and for-each loop have removed the `__has_next__()`
  ...
  [`StopIteration`](/docs/std/iter/StopIteration). This follows Python precedent
...
- The `ImplicitlyBoolable` trait has been removed. This trait enabled types to
  implicitly convert to [`Bool`](/docs/std/builtin/bool/Bool). This behavior was
...
- [`Counter`](/docs/std/collections/counter/Counter) now conforms to `Writable`,
  `Stringable`, and `Representable`.
...
- Added support for [`SIMD`](/docs/std/builtin/simd/SIMD) expressions in `where`
  clauses:
