...
API](/max/rest-api/#POST/v1/batches) is now available with
[Mammoth](/mammoth/).
...
tutorial](/max/develop/serve-custom-model-architectures), with
[example code on GitHub](https://github.com/modular/modular/tree/modular/v26.3/max/examples/custom-models).
...
- Added the [`/health` endpoint](/max/rest-api/#GET/health) for service
...
and into [`max.graph.dim`](/max/api/python/graph). You can still import
them directly from `max.graph`.

- Moved `Shape` out of `max.type` and into
[`max.graph.shape`](/max/api/python/graph). You can still import it
