...
- **Now accepting GPU kernel contributions**
  ...
  the
  [MAX AI kernels contributing guide](https://github.com/modular/modular/blob/modular/v26.3/max/kernels/CONTRIBUTING.md).
...
- **Preview: Mojo interoperability from Python**
  ...
  Python](https://www.mojolang.org/docs/manual/python/mojo-from-python).
...
- [Profile MAX kernel
performance](https://github.com/modular/modular/blob/modular/v26.3/max/docs/kernel-profiling.md):
...
docs for
[calling Mojo from Python](https://www.mojolang.org/docs/manual/python/mojo-from-python).

- Added
[`Graph.add_subgraph()`](/max/api/python/generated/max.graph.Graph#max.graph.Graph.add_subgraph)
to allow for the addition of a subgraph to a graph.

- Added
[`Module.build_subgraph()`](/max/api/python/generated/max.nn.Module#max.nn.Module.build_subgraph)
...
- Added
[`KernelLibrary`](/max/api/python/generated/max.graph.KernelLibrary)
as an argument type for the
[`Graph`](/max/api/python/generated/max.graph.Graph) constructor.

- Added
[`QuantizationConfig`](/max/api/python/generated/max.graph.quantization.QuantizationConfig)
...
[`Module.load_state_dict()`](/max/api/python/generated/max.nn.Module#max.nn.Module.load_state_dict)
...
[`TTSContext`](/max/api/python/generated/max.pipelines.TTSContext),
...
- [`LayoutTensor`](https://www.mojolang.org/docs/layout/layout_tensor/LayoutTensor/)
  now has a
`size` method to get the total number of elements.

- Following our [previous deprecation](/max/changelog/#25-3-engine-mojo-api) of
...
example, with a custom `grayscale` operation written in Mojo:
  ...
  [tutorial to write custom ops for PyTorch](/max/develop/custom-kernels-pytorch),
  and our
  <!-- rumdl-disable-next-line MD013 -->
  [PyTorch custom operation examples](https://github.com/modular/modular/tree/modular/v26.3/max/examples/pytorch_custom_ops),
...
contributing
guide](https://github.com/modular/modular/blob/modular/v26.3/max/kernels/CONTRIBUTING.md).

## Mojo language {#25-4-mojo}

For all the updates to the Mojo language, standard library, and tools, see the
[Mojo changelog](https://www.mojolang.org/releases).
