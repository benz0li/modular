...
[`Module`](/max/api/python/generated/max.nn.Module) APIs are
...
[`Graph`](/max/api/python/generated/max.graph.Graph) APIs.
...
`kbench`](https://github.com/modular/modular/tree/modular/v26.3/max/kernels/benchmarks/autotune#kbench-a-benchmarking-toolkit-for-mojo-kernels),
...
memory is less than 95% full _after_ allocating blocks for that request or if no
...
experience.
  ...
  for backward compatibility in [`max.nn.legacy`](/max/api/python/nn).
...
[`Module`](/max/api/python/generated/max.nn.Module) to compute the
...
[`RotaryEmbedding`](/max/api/python/generated/max.nn.RotaryEmbedding) and
[`TransposedRotaryEmbedding`](/max/api/python/nn).

- Added
[`ArchConfig`](/max/api/python/pipelines.lib.interfaces#max.pipelines.lib.interfaces.ArchConfig)
...
- Added `calculate_virtual_device_count()`,
  ...
  [`max.driver`](/max/api/python/driver).
...
- Renamed `prefill_chunk_size` to `max_batch_input_tokens`
  ...
  in [`PipelineConfig`](/max/api/python/generated/max.pipelines.PipelineConfig)
