...
- Added the `max benchmark` subcommand that runs a suite of benchmarks and
  ...
  [`benchmark_serving.py`](https://github.com/modular/modular/tree/modular/v26.3/benchmark#benchmark-max)
...
- Added `generate()` to
  ...
  and
  [`StandaloneSpeculativeDecodingPipeline`](/max/api/python/generated/max.pipelines.lib.StandaloneSpeculativeDecodingPipeline),
...
- Renamed the `target_num_new_tokens` configuration parameter to
  `prefill_chunk_size`
  in
  [`PipelineConfig`](/max/api/python/generated/max.pipelines.PipelineConfig)
...
- Fixed [`ops.range`](/max/api/python/graph.ops#max.graph.ops.range) to respect
  the `dtype` parameter when using [`Dim`](/max/api/python/graph) objects as
...
- Added [`max.nn.lora`](/max/api/python/nn), providing
  Low-Rank Adaptation (LoRA) support for parameter-efficient fine-tuning of
  neural network models.

- Added [`max.nn.moe`](/max/api/python/nn), implementing
  Mixture of Experts (MoE) layers for scalable model architectures.

- Added [`max.nn.sampling`](/max/api/python/nn),
  containing advanced sampling methods including MinP and rejection sampling
  techniques.

- Added [`max.nn.hooks`](/max/api/python/nn), providing
  debugging and inspection hooks for neural network layers.

- Added attention submodules
  [`max.nn.attention.mask_config`](/max/api/python/nn.attention),
  [`max.nn.attention.multihead_attention`](/max/api/python/nn.attention),
  and
  [`max.nn.attention.multi_latent_attention`](/max/api/python/nn.attention)
...
package](https://www.mojolang.org/docs/manual/install#whats-included).

## MAX kernels {#25-6-kernels}

- Added a leaky ReLU activation function kernel.

- Added a specialized [RMS norm](/max/api/kernels/nn/normalization/rms_norm/)
