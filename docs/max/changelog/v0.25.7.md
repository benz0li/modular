...
GitHub](https://github.com/modular/modular/tree/modular/v26.3/max/python/max)!
...
source](https://github.com/modular/modular/blob/modular/v26.3/max/python/max/entrypoints/pipelines.py).
...
more [in our GitHub
repo](https://github.com/modular/modular/tree/modular/v26.3/max/python/max).
...
[`PagedKVCacheManager`](/max/api/python/generated/max.kv_cache.PagedKVCacheManager)
implementation. During merger, `prefetch()` was renamed `maybe_reserve()`.

- Added
[`NullKVCacheManager`](/max/api/python/generated/max.kv_cache.DummyKVCache)
for compile-only mode, which avoids GPU memory allocation when compiling models
without a physical GPU present.

- Added
[`ResetPrefixCacheBackend`](/max/api/python/kv_cache)
and
[`ResetPrefixCacheFrontend`](/max/api/python/kv_cache)
...
- Changed
[`LoRAConfig.max_num_loras`](/max/api/python/generated/max.pipelines.LoRAConfig#max.pipelines.LoRAConfig.max_num_loras)
...
- Added [`max.nn.comm`](/max/api/python/nn) with `Allreduce` and
...
docs](/max/api/kernels/extensibility/tensor/) moved.
