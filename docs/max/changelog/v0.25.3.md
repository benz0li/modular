...
The
[MAX AI kernels library](https://www.mojolang.org/docs/lib#max-ai-kernels-library)
is a new Mojo API for writing high-performance and portable programs across CPU
and GPU, but it's also
[the source code for our CPU/GPU kernels](https://github.com/modular/modular/tree/modular/v26.3/max/kernels/src).
...
GPUs.
  ...
  GitHub](https://github.com/modular/modular/tree/modular/v26.3/mojo/std/src).
...
- [REST API reference](/max/rest-api/): Although it's not a new API (our
...
[`Weight`](/max/api/python/generated/max.graph.Weight)
...
- Added the [`GroupNorm`](/max/api/python/nn) layer.
...
with the new [`max.nn.Module`](/max/api/python/generated/max.nn.Module)
are the default. That is, `max.nn.LinearV2` is now
[`max.nn.Linear`](/max/api/python/generated/max.nn.Linear), and the
...
[`tensor.Tensor`](/max/api/kernels/extensibility/tensor/tensor/Tensor):
...
[`Graph`](/max/api/python/generated/max.graph.Graph)
...
- New
  [image_pipeline example](https://github.com/modular/modular/tree/modular/v26.3/max/examples/custom_ops)
...
Also see the
[Mojo changelog of GPU changes](https://www.mojolang.org/releases#gpu-changes).
...
kernels](https://github.com/modular/modular/tree/modular/v26.3/max/kernels/src).
