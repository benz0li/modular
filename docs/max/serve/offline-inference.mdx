...
github_url: https://github.com/modular/modular/blob/modular/v26.3/max/examples/offline-inference/basic.py
---
...
You'll want to use offline inference in scenarios where you want to perform
model inference without the need for a separate model inference server.
Typically this includes where you have to process a batch of inputs
concurrently.
...
[`LLM`](/max/api/python/generated/max.entrypoints.llm.LLM) class which provides
...
[`max_length`](/max/api/python/generated/max.pipelines.MAXModelConfig#max.pipelines.MAXModelConfig.max_length)
as kwargs to
[`MAXModelConfig`](/max/api/python/generated/max.pipelines.MAXModelConfig), and
runtime settings such as
[`max_num_steps`](/max/api/python/generated/max.pipelines.lib.PipelineRuntimeConfig#max.pipelines.lib.PipelineRuntimeConfig.max_num_steps)
to
[`PipelineRuntimeConfig`](/max/api/python/generated/max.pipelines.lib.PipelineRuntimeConfig),
then pass these sub-configs to
[`PipelineConfig`](/max/api/python/generated/max.pipelines.PipelineConfig). Then
call
[`generate()`](/max/api/python/generated/max.entrypoints.llm.LLM#max.entrypoints.llm.LLM.generate)
...
capabilities, see [Image and video to text](/max/inference/image-to-text).
...
2. Create a file named `main.py` with the following code:
    ...
    You need both a valid Hugging Face token and model access approval to serve
    Gemma 3. To create a Hugging Face user access token, see
    [Access Tokens](https://huggingface.co/settings/tokens). You can request
    model access through the
    [Gemma 3 Hugging Face model
    repository](https://huggingface.co/google/gemma-3-12b-it).
...
use a different model, see our [supported models](/max/models/). This example
